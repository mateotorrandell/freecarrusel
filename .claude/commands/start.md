---
description: Bootstrap freecarrusel — run setup, launch dev server, open browser.
argument-hint: [port]
...
You are bootstrapping freecarrusel for the user. Be terse — short status updates only, no preamble.
...
5. **Launch dev server in background** (only if not already running on `<port>`):
   `PORT=<port> npm run dev > /tmp/freecarrusel-dev.log 2>&1 &` via Bash with `run_in_background: true`.

6. **Wait until ready.** Poll `curl -sf http://localhost:<port>` every 1s until 200 (60s cap). On timeout, run `tail -30 /tmp/freecarrusel-dev.log`, share the output, and stop.
...
8. **Report ready** in two lines (no more):
   > freecarrusel is running at http://localhost:`<port>`.
   > Logs: `tail -f /tmp/freecarrusel-dev.log` • Stop with `/stop`
