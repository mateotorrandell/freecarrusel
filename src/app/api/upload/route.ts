...
export async function POST(request: Request) {
  try {
    ...
    // SVG is how most sites ship their logo, so refusing it outright meant the
    // agent settled for a favicon. Accept it but RASTERISE immediately: the SVG
    // is never stored and never served, so the XSS surface stays closed.
    const isSvg =
      ext === ".svg" ||
      file.type === "image/svg+xml" ||
      /^\s*(<\?xml|<svg)/i.test(
        new TextDecoder().decode(buffer.slice(0, 200))
      );
    if (isSvg) {
      const id = generateId();
      await mkdir(UPLOAD_DIR, { recursive: true });
      const png = await sharp(Buffer.from(arrayBuffer), { density: 384 })
        .resize(1080, 1080, { fit: "inside", withoutEnlargement: false })
        .png()
        .toBuffer();
      const filename = `${id}.png`;
      await writeFile(path.join(UPLOAD_DIR, filename), png);
      return NextResponse.json({
        id,
