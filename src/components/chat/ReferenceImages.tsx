...
import { ImagePlus, X, Scissors, Loader2 } from "lucide-react";
        ...
        <div className="flex gap-2 px-4 pb-3 overflow-x-auto">
          {images.map((img) => (
            <div key={img.id} className="oc-enter-pop relative group shrink-0">
              ...
              {/* Cut out the subject — runs locally, adds a transparent copy */}
              <button
                onClick={async () => {
                  setCutting(img.id);
                  try {
                ...
                className="absolute -bottom-1 -right-1 h-4 w-4 rounded-full bg-foreground text-background flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-100"
