...
/**
 * `expectedHtml` turns the write into a compare-and-set: the caller states
 * which version it was editing, and the update is refused if the stored slide
 * has moved on since. The comparison happens here, inside the same lock as the
 * write — checking in the route would leave a window where the agent's save
...
export async function updateSlide(
  ...
  expectedHtml?: string
): Promise<Slide | null | { conflict: Slide }> {
  const data = await load();
  const carousel = data.carousels.find((c) => c.id === carouselId);
  if (!carousel) return null;
  const slide = carousel.slides.find((s) => s.id === slideId);
  if (!slide) return null;
  if (typeof expectedHtml === "string" && slide.html !== expectedHtml) {
    return { conflict: slide };
  }
