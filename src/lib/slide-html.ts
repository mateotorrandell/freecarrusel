...
import { SLIDE_EDITOR_SCRIPT } from "./slide-editor-script";
...
export function wrapSlideHtml(
  ...
  options?: { inlineFontCss?: string; editable?: boolean }
): string {
  ...
  // Edit mode only. Preview and export never pass `editable`, so their output
  // is byte-identical to before — the rendering contract is unchanged.
  const editorBlock = options?.editable
    ? `<script>${SLIDE_EDITOR_SCRIPT}</script>`
    : "";
