...
function seedDataFiles() {
  ...
  const seeds = {
    ...
    "settings.json": { language: "es", updatedAt: "" },
  };
  ...
  // Permission set for the chat subprocess. /api/chat runs the agent with
  // --setting-sources local so it doesn't inherit the host's Claude Code config
  // (which makes it act like a dev agent on this repo instead of the design
  // engine). That isolation also drops the host's allowlist, so curl — the only
  // way the agent writes slides — has to be granted back here.
  const agentSettings = path.join(dataDir, ".agent-settings.json");
  if (!fs.existsSync(agentSettings)) {
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          permissions: {
            // curl is the agent's only write path: it POSTs slides to the local
            // API. Everything that would let it edit state directly is denied —
            // reading data/ at all tempts it into rewriting carousels.json by
            // hand instead of going through the API, which races the app.
            allow: [
              "Bash(curl:*)",
              "Bash(node:*)",
              "Bash(cat:*)",
              // Its own scratch dir. Note the folder is NOT dotted: permission
              // globs don't match hidden directories, which silently denied the
              // agent write access to the very workspace we gave it.
    ...
    log(`  Created ${path.relative(ROOT, agentSettings)}`);
  }
}
