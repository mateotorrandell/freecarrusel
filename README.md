...
There is a desktop build — one window, no terminal, your work in the usual place
for your operating system.

```bash
npm run desktop:build      # builds an installer for the machine you are on
npm run desktop:dev        # runs the shell against a local build
```

Installers for both platforms are produced by CI (`.github/workflows/desktop.yml`):
push a `v*` tag, or run the workflow by hand, and it attaches a `.exe` and a
`.dmg` to a draft release. A `.dmg` can only be assembled on macOS, which is
why this goes through CI rather than one machine.

**What the shell does.** It starts the app's own server on a private port, opens
a window at it, and shuts it down on quit. Your carousels and images move out of
the project folder and into the standard per-user location, so an update never
touches them:

| | |
|---|---|
| **Windows** | `%APPDATA%reecarrusel` |
| **macOS** | `~/Library/Application Support/freecarrusel` |
| **Linux** | `~/.config/freecarrusel` |

**PNG export doesn't ship a second browser.** Electron already contains
Chromium, so the desktop build renders slides in an offscreen window instead of
downloading Puppeteer's own copy. That is ~670 MB the installer doesn't carry,
and an export takes about a second instead of several.

**The assistant still needs the Claude Code CLI.** It cannot be bundled: it is a
separate tool that signs in with your own account, which is also why nothing is
billed through this app. On first run freecarrusel checks for it and, if it's
missing, tells you how to install it. The editor works either way — you just
won't see the chat.
...
- **Windows.** With Smart App Control enabled — the default on many new Windows
  11 machines — an unsigned installer is **blocked outright**, not merely warned
  about. Turning that setting off is a one-way switch that needs a Windows
  reset, so it is not something to ask users to do. Where Smart App Control is
  off, SmartScreen shows a notice you can pass with *More info → Run anyway*.
- **macOS.** Gatekeeper refuses the first launch. Right click the app →
  **Open** → **Open** gets past it, once.

Signing fixes both: an Apple Developer account (~USD 99/year) covers macOS, and
a Windows certificate is a separate, more expensive one. Until then, running
from source is the path with no friction at all.
